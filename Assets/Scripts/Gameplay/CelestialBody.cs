...
public class CelestialBody : MonoBehaviour
{
    ...
    public float radius;      // Rayon de l'objet (pour l'échelle)
    public float density;
    public float starLuminosity = 0; //0 for planet, applied to stars
    ...
    public float orbitalRadius = 10f;         // Rayon de l'orbite en unités
...
private (float azimuth, float elevation, float distance) CalculatePositionData()
{
    ...
    public float CalculateLuminosity(Transform star, Transform observer)
    {
        ...
        float distanceToObserver = Vector3.Distance(transform.position, observer.position);
        ...
        {
            return star.GetComponent<CelestialBody>().starLuminosity * albedo * phase / (distanceToObserver * distanceToObserver);
        }
    }
